  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-16">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 text-center py-24">
        {/* Badge */}
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-600/10 border border-blue-500/20 text-blue-400 text-xs font-semibold mb-8">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16">
          <Link href="/login"
            className="w-full sm:w-auto px-8 py-4 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-base transition-all shadow-lg shadow-blue-600/20 flex items-center justify-center gap-2">
            Start Free 14-Day Trial
            <ArrowRight className="w-4 h-4" />
          </Link>
          <a href="#how-it-works"
            className="w-full sm:w-auto px-8 py-4 rounded-xl border border-slate-600 hover:border-slate-400 text-slate-300 font-semibold text-base transition-all flex items-center justify-center gap-2">
            See How It Works
          </a>
        </div>
...
function StatsBar() {
  ...
  return (
    <section className="bg-slate-900/60 border-y border-slate-700/40 py-10">
    ...
    <section className="py-24 bg-[#0a0f1e]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="grid md:grid-cols-2 gap-16 items-center">
          <div>
            <div className="inline-block px-3 py-1 rounded-full bg-red-500/10 border border-red-500/20 text-red-400 text-xs font-semibold mb-6">
              The Problem
            </div>
            <h2 className="text-3xl sm:text-4xl font-bold text-white mb-6 leading-tight">
              Indian SMBs are under attack.<br />
              <span className="text-red-400">But can't afford protection.</span>
            </h2>
          ...
          <div className="rounded-2xl bg-slate-800/40 border border-slate-700/50 p-6">
            <h3 className="text-sm font-semibold text-slate-400 mb-5 uppercase tracking-wider">Competitor Comparison</h3>
            <div className="space-y-3">
              {[
                ...
                <div key={r.name} className={`flex items-center gap-3 p-3 rounded-lg text-xs ${r.highlight ? "bg-blue-600/10 border border-blue-500/20" : "bg-slate-900/50"}`}>
                  <span className={`flex-1 font-medium ${r.highlight ? "text-blue-300" : "text-slate-300"}`}>{r.name}</span>
                  <span className={`w-24 text-right ${r.highlight ? "text-blue-400 font-bold" : "text-slate-400"}`}>{r.cost}</span>
                  <Tick val={r.india} />
                  <Tick val={r.certin} />
                  <Tick val={r.smb} />
                </div>
...
function Tick({ val }: { val: boolean | string }) {
  if (val === true) return <CheckCircle2 className="w-4 h-4 text-emerald-400 flex-shrink-0" />;
  if (val === "partial") return <span className="w-4 h-4 text-yellow-400 flex-shrink-0 text-center font-bold">~</span>;
  return <XCircle className="w-4 h-4 text-red-400/50 flex-shrink-0" />;
}
    ...
    <section id="how-it-works" className="py-24 bg-slate-900/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-16">
          <div className="inline-block px-3 py-1 rounded-full bg-blue-500/10 border border-blue-500/20 text-blue-400 text-xs font-semibold mb-4">
            How It Works
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {steps.map((s) => (
            <div key={s.step} className="rounded-2xl bg-slate-800/40 border border-slate-700/50 p-8 hover:border-slate-600/50 transition-all">
    ...
    <section id="features" className="py-24 bg-[#0a0f1e]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5">
          {modules.map((m) => (
            <div key={m.title} className="rounded-xl bg-slate-800/40 border border-slate-700/40 p-5 hover:border-slate-500/50 hover:-translate-y-0.5 transition-all">
              <div className={`w-10 h-10 rounded-lg border flex items-center justify-center mb-4 ${colorMap[m.color]}`}>
                <m.icon className="w-5 h-5" />
              </div>
              <h3 className="font-semibold text-white text-sm mb-2">{m.title}</h3>
              <p className="text-xs text-slate-400 leading-relaxed mb-4">{m.desc}</p>
              <div className="flex flex-wrap gap-1">
                {m.tags.map(t => (
                  <span key={t} className="px-2 py-0.5 rounded-full bg-slate-700/60 text-slate-400 text-[10px]">{t}</span>
                ))}
              </div>
            </div>
          ))}
...
function ComplianceSection() {
  ...
  return (
    <section id="compliance" className="py-24 bg-slate-900/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="grid md:grid-cols-2 gap-16 items-start">
          <div>
            <div className="inline-block px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-semibold mb-6">
              CERT-In Native
            </div>
              ...
              {[
                ...
                <div key={i.label} className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <span className="text-sm text-slate-400"> — {i.desc}</span>
                  </div>
                </div>
              ))}
            </div>
            <Link href="/login" className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-semibold text-sm transition-all">
          ...
          <div>
            <div className="rounded-2xl bg-slate-800/40 border border-slate-700/50 p-6">
              <div className="flex items-center justify-between mb-5">
                <h3 className="font-semibold text-white text-sm">15 CERT-In MSME Controls — CISG-2024-03</h3>
                <span className="text-xs text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded">All Covered</span>
              </div>
              <div className="grid grid-cols-1 gap-2">
                {controls.map((c, i) => (
                  <div key={c} className="flex items-center gap-3 py-2 border-b border-slate-700/30 last:border-0">
                    <span className="text-xs font-mono text-slate-600 w-6 flex-shrink-0">{String(i + 1).padStart(2, "0")}</span>
                    <span className="text-sm text-slate-300 flex-1">{c}</span>
    ...
    <section id="pricing" className="py-24 bg-[#0a0f1e]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-5 mb-12">
          {plans.map((p) => (
            <div key={p.name} className={`rounded-2xl bg-slate-800/40 border ${colorBorder[p.color]} p-6 flex flex-col relative ${p.color === "blue" ? "ring-1 ring-blue-500/30" : ""}`}>
              {p.tag && (
              ...
              <div className="mb-5">
                ...
                {p.hindi && <div className="text-slate-500 text-xs mt-0.5">{p.hindi}</div>}
              </div>
              <div className="mb-2">
                <span className="text-3xl font-bold text-white">₹{p.price}</span>
                <span className="text-slate-400 text-sm">/{p.period}</span>
              </div>
              <div className="text-xs text-blue-400 mb-4">{p.employees}</div>
              ...
              <Link href={p.cta === "Contact Sales" ? "#contact" : "/login"}
        ...
        {/* Custom govt plan */}
        <div className="rounded-2xl bg-gradient-to-r from-blue-600/10 to-purple-600/10 border border-blue-500/20 p-8 flex flex-col md:flex-row items-center justify-between gap-6">
          <div className="flex items-center gap-4">
            <Building2 className="w-10 h-10 text-blue-400 flex-shrink-0" />
            <div>
              <div className="font-bold text-white text-lg">Government / PSU Custom Plan</div>
              <div className="text-slate-400 text-sm mt-1">Dedicated instance · India data residency certification · Custom CERT-In integration · On-premise option · 99.9% SLA</div>
            </div>
          </div>
          <a href="#contact" className="flex-shrink-0 px-6 py-3 rounded-xl border border-blue-500/40 hover:border-blue-400 text-blue-400 hover:text-blue-300 font-semibold text-sm transition-all">
            Contact for Pricing
          </a>
        </div>
      </div>
    ...
    <section className="py-24 bg-slate-900/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-400">From manufacturing to SaaS, exporters to government vendors</p>
        </div>
        <div className="grid md:grid-cols-2 gap-5">
          {reviews.map((r) => (
            <div key={r.name} className="rounded-2xl bg-slate-800/40 border border-slate-700/40 p-6">
        ...
        <div className="space-y-3">
          {faqs.map((f, i) => (
            <div key={i} className="rounded-xl border border-slate-700/50 bg-slate-800/30 overflow-hidden">
              <button className="w-full px-6 py-4 flex items-start gap-4 text-left" onClick={() => setOpen(open === i ? null : i)}>
                <span className="flex-1 text-sm font-semibold text-white">{f.q}</span>
                {open === i ? <ChevronUp className="w-4 h-4 text-slate-400 flex-shrink-0 mt-0.5" /> : <ChevronDown className="w-4 h-4 text-slate-400 flex-shrink-0 mt-0.5" />}
              </button>
              {open === i && (
                <div className="px-6 pb-5 text-sm text-slate-400 leading-relaxed border-t border-slate-700/30 pt-4">
                  {f.a}
                </div>
              )}
            </div>
    ...
    <section id="contact" className="py-24 bg-slate-900/30">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div className="grid md:grid-cols-2 gap-16 items-center">
          ...
          <div className="rounded-2xl bg-slate-800/40 border border-slate-700/50 p-8">
            <h3 className="font-bold text-white mb-6">Request a Demo</h3>
            <div className="space-y-4">
              {[
              ...
              <button className="w-full py-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-semibold text-sm transition-all">
                Request Demo →
              </button>
            </div>
          </div>
...
function CTABanner() {
  return (
    <section className="py-20 bg-gradient-to-r from-blue-900/40 to-purple-900/20 border-y border-blue-500/10">
