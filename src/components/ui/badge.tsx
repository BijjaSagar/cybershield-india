...
const complianceConfig: Record<ComplianceStatus, string> = {
  ...
  partial:         "bg-yellow-500/20 text-yellow-400 border border-yellow-500/40",
  "non-compliant": "bg-red-500/20 text-red-400 border border-red-500/40",
};
...
export function StatusDot({ active }: { active: boolean }) {
  return (
    <span className={cn("inline-block w-2 h-2 rounded-full", active ? "bg-emerald-400 pulse-dot" : "bg-slate-600")} />
  );
}
